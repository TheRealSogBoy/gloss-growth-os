...
const regex = /const handleGastoBoveda = async \(e\) => \{[\s\S]*?setFormGastoBoveda\(\{ concepto: '', categoria: 'Herramientas y Software', monto: '', fecha: new Date\(\)\.toISOString\(\)\.split\('T'\)\[0\], metodo: '[^']+' \}\);\s*\};/;
...
if(fj.match(regex)) {
   ...
   console.log('✅ Applied handleGastoBoveda patch');
